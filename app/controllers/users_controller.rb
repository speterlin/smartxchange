...
class UsersController < ApplicationController
  ...
  def index
    ...
    if params[:language]
      ...
      @users = User.search(params[:search], misspellings: {below: 2, prefix_length: 2}, includes: [:linkedin], page: params[:page], per_page: 12) # according to chatgpt could refactor: Doesn't support combined filters (e.g. language=French&search=Engineering)
    else
