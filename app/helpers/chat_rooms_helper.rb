module ChatRoomsHelper
  ...
  # quick fix using select instead of where because that chat_room is loaded with notifications and we don't have to query the database again
  def chat_room_count_unread(chat_room, user)
    # chat_room.notifications.where(read: false, notified_id: user.id).count
    chat_room.notifications.select {|n| !n.read && n.notified_id == user.id}.count
  end
  ...
  def chat_room_notification_check(chat_room, receiver)
    print('chat room notification check')
    # working ok this is a bit buggy (keeping original below): return false if chat_room.notifications.select {|notification| notification.read == false && notification.notified_id == receiver.id}.count > 0
  ...
  def chat_room_mark_read(chat_room, notified)
    ...
    # in ruby 3.4 rails 7.2 associations boolean values in db sqlite3 are not integer (fixed with migration) (are loaded: chat_room.notifications is already loaded)
    chat_room.notifications.select {|n| !n.read && n.notified_id == notified.id}.each do |n|
      n.update(read: true)
    end
