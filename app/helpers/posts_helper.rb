module PostsHelper
  ...
  def post_notification_check(vote_or_comment_or_follow_or_post_update_or_comment_update, post, notified)
    return false if vote_or_comment_or_follow_or_post_update_or_comment_update.is_a?(Comment) && post.notifications.where(read: false, notified_id: notified.id, sourceable_type: 'Comment', sourceable_id: vote_or_comment_or_follow_or_post_update_or_comment_update.id).count > 0
    return false if vote_or_comment_or_follow_or_post_update_or_comment_update.is_a?(Vote) && post.notifications.where(read: false, notified_id: notified.id, sourceable_type: 'Vote').count > 0
    return false if vote_or_comment_or_follow_or_post_update_or_comment_update.is_a?(Follow) && post.notifications.where(read: false, notified_id: notified.id, sourceable_type: 'Follow').count > 0
    return false if vote_or_comment_or_follow_or_post_update_or_comment_update.is_a?(Post) && post.notifications.where(read: false, notified_id: notified.id, sourceable_type: 'Post').count > 0
    true
  end
  ...
  def post_create_notifications(vote_or_comment_or_follow_or_post_update_or_comment_update, post)
    ...
    if post.followers.any?
      post.followers.each do |follower|
        ...
        post_create_notification(vote_or_comment_or_follow_or_post_update_or_comment_update, post, follower)
      end
    end
  end
